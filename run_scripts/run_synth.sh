...
do
    ...
    TRAINING_ARGS='--add_uniform_prior --no_edge_prior 0.2 --batch_size 256 --lr 1e-4 --use_adam --num_epochs 1500 --lr_decay_factor 0.9 --lr_decay_steps 100 --normalize_kl --normalize_nll --tune_on_nll --val_teacher_forcing --teacher_forcing_steps -1' #--train_hard_sample'
    mkdir -p $WORKING_DIR
    CUDA_VISIBLE_DEVICES=$GPU python -u dnri/experiments/small_synth_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS |& tee "${WORKING_DIR}results.txt"
